...
================================= */
$(function () {
  window.room_submission = {}; // 因原始格式不好排資料，所以需要重排過
  ...
  $.getJSON(url, function (data) {
    $.each(data, function (v, d) {
      ...
      time = time + '~' + padLeft('' + date_end.getHours(), 2) + ':' + padLeft('' + date_end.getMinutes(), 2);

      if (!room_submission[day]) { room_submission[day] = {}; }
      if (!room_submission[day][d['room']]) { room_submission[day][d['room']] = []; }
      // 重新定義一個Object 存放解完的資料
      room_submission[day][d['room']].push({
    ...
    $.each(room_submission, function (v_day, d_room) {
      ...
      $('.class_room').each(function () { // 設定每個議程的顯示位置與時間(也就是從幾點開始到幾點)
        ...
        $(this).find('.class_sub').each(function () {
          ...
          $(this).css({'height': ((time_end - time_start) * 2) + 'px'}); // 空間高度就是開始時間減去結束時間
